...
def get_netbox_docker_version_tag(netbox_version):
    ...
    if (major, minor) == (2, 9):
        ...
        raise ValueError("Versions below 2.7 are not currently supported")
...
def netbox_docker_repo_dirpaths(pytestconfig, git_toplevel):
    ...
    for netbox_version in pytestconfig.option.netbox_versions:
        ...
        repo_fpath = os.path.join(git_toplevel, ".netbox-docker-%s" % str(repo_version_tag))
        if os.path.isdir(repo_fpath):
            subp.check_call(["git", "fetch"], cwd=repo_fpath, stdout=subp.PIPE, stderr=subp.PIPE)
            subp.check_call(
                ["git", "pull", "origin", "release"], cwd=repo_fpath, stdout=subp.PIPE, stderr=subp.PIPE,
            )
        else:
            subp.check_call(
                ["git", "clone", "https://github.com/netbox-community/netbox-docker", repo_fpath],
                cwd=git_toplevel,
                stdout=subp.PIPE,
                stderr=subp.PIPE,
            )
        ...
        subp.check_call(
            ["git", "checkout", repo_version_tag], cwd=repo_fpath, stdout=subp.PIPE, stderr=subp.PIPE,
        )
...
def clean_docker_objects():
    ...
    for line in subp.check_output(["docker", "ps", "-a"]).decode("utf-8").splitlines():
        ...
        if words[-1].startswith(DOCKER_PROJECT_PREFIX):
            subp.check_call(["docker", "rm", "-f", words[0]], stdout=subp.PIPE, stderr=subp.PIPE)

    # clean up any volumes
    for line in subp.check_output(["docker", "volume", "list"]).decode("utf-8").splitlines():
        words = line.split()
        if not words:
            continue
        ...
        if words[-1].startswith(DOCKER_PROJECT_PREFIX):
            subp.check_call(
                ["docker", "volume", "rm", "-f", words[-1]], stdout=subp.PIPE, stderr=subp.PIPE,
            )

    # clean up any networks
    for line in subp.check_output(["docker", "network", "list"]).decode("utf-8").splitlines():
        words = line.split()
        if not words:
            continue
        ...
        if words[1].startswith(DOCKER_PROJECT_PREFIX):
            subp.check_call(
                ["docker", "network", "rm", words[1]], stdout=subp.PIPE, stderr=subp.PIPE,
            )
...
def docker_compose_file(pytestconfig, netbox_docker_repo_dirpaths):
    ...
    for (netbox_docker_repo_dirpath, netbox_versions,) in netbox_docker_repo_dirpaths.items():
        compose_source_fpath = os.path.join(netbox_docker_repo_dirpath, "docker-compose.yml")
        ...
        for netbox_version in netbox_versions:
            ...
            for service_name in compose_data["services"].keys():
                ...
                if service_name in ["netbox", "netbox-worker"]:
                    ...
                    new_services[new_service_name]["image"] = "netboxcommunity/netbox:v%s" % netbox_version

                # set the network and an alias to the proper short name of the container
                # within that network
                new_services[new_service_name]["networks"] = {docker_network_name: {"aliases": [service_name]}}

                # fix the naming of any dependencies
                if "depends_on" in new_services[new_service_name]:
                    new_service_dependencies = []
                    for dependent_service_name in new_services[new_service_name]["depends_on"]:
                        new_service_dependencies.append(
                            "netbox_v%s_%s" % (docker_netbox_version, dependent_service_name,)
                        )
                    ...
                    for volume_config in new_services[new_service_name]["volumes"]:
                        ...
                        if "/" in source:
                            if volume_config.startswith("./"):
                                ...
                                volume_source, volume_dest = volume_config.split(":", maxsplit=1)
                                volume_source = os.path.join(netbox_docker_repo_dirpath, volume_source[2::])
                                new_volumes.append(":".join([volume_source, volume_dest]))
                            else:
                                new_volumes.append(volume_config)
                        else:
                            new_volumes.append(
                                "%s_v%s_%s" % (DOCKER_PROJECT_PREFIX, docker_netbox_version, volume_config,)
                            )
            ...
            for volume_name, volume_config in compose_data["volumes"].items():
                new_volumes["%s_v%s_%s" % (DOCKER_PROJECT_PREFIX, docker_netbox_version, volume_name,)] = volume_config
            compose_data["volumes"] = new_volumes

            compose_output_fpath = os.path.join(netbox_docker_repo_dirpath, "docker-compose-v%s.yml" % netbox_version,)
...
def populate_netbox_object_types(nb_api, devicetype_library_repo_dirpath, faker):
    ...
    for device_role_name in DEVICE_ROLE_NAMES:
        ...
        nb_api.dcim.device_roles.create(
            name=device_role_name, slug=device_role_name.lower().replace(" ", "-"), **extra_kwargs
        )


@pytest.fixture(scope="session")
def netbox_service(
    pytestconfig, docker_ip, docker_services, devicetype_library_repo_dirpath, request, faker,
):
    ...
    populate_netbox_object_types(
        nb_api=nb_api, devicetype_library_repo_dirpath=devicetype_library_repo_dirpath, faker=faker,
    )
...
def pytest_generate_tests(metafunc):
    ...
    if "netbox_service" in metafunc.fixturenames:
        ...
        metafunc.parametrize(
            "netbox_service", metafunc.config.getoption("netbox_versions"), ids=id_netbox_service, indirect=True,
        )
